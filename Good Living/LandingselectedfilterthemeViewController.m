...
#import "LandingselectedfilterthemeViewController.h"
#import "Customcell.h"
#import "FilterController.h"

@interface LandingselectedfilterthemeViewController ()
@end

@implementation LandingselectedfilterthemeViewController
...
- (void)viewDidLoad
{
    ...
    {
        if ([UD objectForKey:@"LandingSelectedCatogories"])
        {
            NSData *data =[UD objectForKey:@"LandingSelectedCatogories"];
            cellSelected=[NSKeyedUnarchiver unarchiveObjectWithData:data];
        }
    }
    else if ([_tableFor isEqualToString:@"areas"])
    {
        if ([UD objectForKey:@"LandingSelectedAreas"])
        {
            NSData *data =[UD objectForKey:@"LandingSelectedAreas"];
            cellSelected=[NSKeyedUnarchiver unarchiveObjectWithData:data];
        }
    }
    ...
    {
        if ([UD objectForKey:@"LandingSelectedCuisine"])
        {
            NSData *data =[UD objectForKey:@"LandingSelectedCuisine"];
            cellSelected=[NSKeyedUnarchiver unarchiveObjectWithData:data];
        }
    }
...
- (UITableViewCell *) tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else if ([_DataString rangeOfString:cell.textLabel.text].location!= NSNotFound)
    {
        cell.accessoryType = UITableViewCellAccessoryCheckmark;
    }
...
{ if ([_tableFor isEqualToString:@"themes"])
{
    ...
    if (indexPath.row==0)
    {
        ...
        if ([_tableFor isEqualToString:@"catagories"])
            [UserDefault setObject:data forKey:@"LandingSelectedCatogories"];
        else if ([_tableFor isEqualToString:@"areas"])
            [UserDefault setObject:data forKey:@"LandingSelectedAreas"];
        //        else if([_tableFor isEqualToString:@"PreferencesAreas"])
        //            [UserDefault setObject:data forKey:@"PreferencesAreas"];
        else if ([_tableFor isEqualToString:@"Cuisine"])
            [UserDefault setObject:data forKey:@"LandingSelectedCuisine"];
        return;
    }
    else
    {
