...
@interface EventInviteesController ()

@end

@implementation EventInviteesController
...
- (void)viewDidLoad
{
    ...
    //GL_Invites_Listing
    [Flurry logEvent:@"GL Invites"];
    [Flurry logPageView];
    
    self.screenName = @"GL Invites";
...
-(void)getTheGiftOfWeek
{
    ...
    @try {
        NSString *urlString = glInvitesURL;
        ...
        if (data)
        {
            ...
            if ([selectIDArray count]==0)
            {
                ...
                Nodata.text=@"We’re sorry! There are no invites at this time.  Check back again soon!";
                ...
                lable.text = @"Win exclusive invitation to signature events.";
