...
- (void)viewDidLoad
{
    ...
    //GL_Traveller_Listing
    [Flurry logEvent:@"GL Traveller"];
    [Flurry logPageView];

    self.screenName = @"GL Traveller";
...
-(void)getTheGiftOfWeek
{
    ...
    @try {
        ...
        NSString *urlString = glTravellerURL;
        ...
        if (data)
        {
            ...
            for (int i = 0; i < [DataArray count]; i++)
            {
                ...
                NSString *dateStr = [[DataArray objectAtIndex:i] valueForKey:@"Participation_Last_Date__c"];
                ...
                if([date compare:today]==NSOrderedSame || [date compare:today]==NSOrderedDescending)
                {
            ...
            if ([selectIDArray count]==0)
            {
                ...
                Nodata.text=@"We’re sorry! There are no event competitions at this time.  Check back again soon!";
                Nodata.font = [UIFont fontWithName:@"Helvetica Neue" size:14];
                Nodata.textAlignment=NSTextAlignmentCenter;
                
                UIImageView *Logoimage = [[UIImageView alloc] init];
                Logoimage.image = [UIImage imageNamed:@"2trav.png"];
                [self.view addSubview:Logoimage];
                
                UILabel *lable = [[UILabel alloc] init];
                lable.text = @"Win amazing 'travel experiences', every fortnight.";
